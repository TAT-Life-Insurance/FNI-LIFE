...
public class ShortTermMonthlyReportExcel {

	private XSSFWorkbook wb;

	public ShortTermMonthlyReportExcel() {
		load();
	}
	...
	public void generate(OutputStream op, List<ShortEndownLifeMonthlyReportDTO> shortEndownLifeMonthlyReportList) {
		try {
			...
			Date date = shortEndownLifeMonthlyReportList.get(0).getPaymentDate();
			...
			cell.setCellValue(year + " " + MyanmarLanguae.getMyanmarLanguaeString("YEAR_MYANMAR_002") + " " + month + " "
					+ MyanmarLanguae.getMyanmarLanguaeString("MONTH_MYANMAR_003") + MyanmarLanguae.getMyanmarLanguaeString("SHORTENDOWNMENT_MONTHLY_HEADER_001"));
			...
			for (ShortEndownLifeMonthlyReportDTO monthlyDto : shortEndownLifeMonthlyReportList) {
				i = i + 1;
				index = index + 1;
