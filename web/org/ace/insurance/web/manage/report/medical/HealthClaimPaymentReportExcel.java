...
public class HealthClaimPaymentReportExcel {
	private XSSFWorkbook wb;

	public HealthClaimPaymentReportExcel() {
		load();
	}

	private void load() {
		try {
			InputStream inp = this.getClass().getResourceAsStream("/report-template/medical/Health_Claim_Payment_Report.xlsx");
		...
		try {
			...
			for (HealthDailyIncomeReportDTO report : hdList) {
				if (report.getBranch() == null) {
					cell.setCellValue(ApplicationSetting.getCompanyLabel() + " \n \n Medical Claim Payment Report ( All )");
					break;
				} else {
					cell.setCellValue(ApplicationSetting.getCompanyLabel() + " \n \n Medical Claim Payment Report ( " + report.getBranch() + " )");
					break;
				}
			}
