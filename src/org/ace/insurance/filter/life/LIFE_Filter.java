...
public class LIFE_Filter extends BasicDAO implements ILIFE_Filter {
	...
	public List<LPLC001> find(CRIA002 criteria) throws DAOException {
		...
		try {
			...
			queryString
					.append("SELECT l.id, l.policyNo, l.lifeProposal.proposalNo, l.agent, l.customer, l.organization, "
			...
			if (!isEmpty(criteria.getAgentId())) {
				queryString.append(" AND l.agent.id = '"
						+ criteria.getAgentId() + "'");
			}
			if (!isEmpty(criteria.getStartDate())) {
				queryString.append(" AND l.commenmanceDate >= '"
						+ criteria.getSQLStartDate() + "'");
			}
			if (!isEmpty(criteria.getEndDate())) {
				queryString.append(" AND l.commenmanceDate <= '"
						+ criteria.getSQLEndDate() + "'");
			}
			if (!isEmpty(criteria.getCustomerId())) {
				queryString.append(" AND l.customer.id = '"
						+ criteria.getCustomerId() + "'");
			}
			if (!isEmpty(criteria.getOrganizationId())) {
				queryString.append(" AND l.organization.id = '"
						+ criteria.getOrganizationId() + "'");
			}
			if (!isEmpty(criteria.getSaleManId())) {
				queryString.append(" AND l.saleMan.id = '"
						+ criteria.getSaleManId() + "'");
			}
			if (!isEmpty(criteria.getBranchId())) {
				queryString.append(" AND l.branch.id = '"
						+ criteria.getBranchId() + "'");
			}
			if (!isEmpty(criteria.getProductId())) {
				queryString.append(" AND pi.product.id = '"
						+ criteria.getProductId() + "'");
			}
			if (!isEmpty(criteria.getReferenceNo())) {
				queryString.append(" AND l.policyNo = '"
			...
			for (Object[] b : objectList) {
				...
				if (resultMap.containsKey(policyNo)) {
					...
					resultMap.put(policyNo, new LPLC001(id, policyNo,
							proposalNo, agent, customer, branch,
							premium, sumInsured, paymenttype, submittedDate));
				} else {
					resultMap.put(policyNo, new LPLC001(id, policyNo,proposalNo,  agent, customer, branch,
							premium, sumInsured, paymenttype, submittedDate));
				}
			}
			em.flush();
		...
		RegNoSorter<LPLC001> regNoSorter = new RegNoSorter<LPLC001>(
				new ArrayList<LPLC001>(resultMap.values()));
