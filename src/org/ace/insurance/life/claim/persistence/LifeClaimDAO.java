...
public class LifeClaimDAO extends BasicDAO implements ILifeClaimDAO {
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public List<LCL001> findByEnquiryCriteria(LCL001 criteria) throws DAOException {
		...
		try {
			...
			queryString.append("SELECT DISTINCT l.POLICYID, l.CLAIMROLE, " + "l.CLAIMINSUREPERSONID, l.SUBMITTEDDATE, l.ID "
					+ "FROM LIFECLAIM_REQUEST l INNER JOIN LIFECLAIM_REQUEST_BENEFICIARY b " + "ON l.ID = b.LIFECLAIMREQUESTID ");
			if (criteria.getPolicyId() != null) {
				queryString.append(" AND l.POLICYID = '" + criteria.getPolicyId() + "'");
			}
			if (criteria.getClaimDate() != null) {
				queryString.append(" AND l.CLAIMDATE = '" + new java.sql.Timestamp(criteria.getClaimDate().getTime()) + "'");
			}
			if (criteria.getSubmittedDate() != null) {
				queryString.append(" AND l.SUBMITTEDDATE = '" + new java.sql.Timestamp(criteria.getSubmittedDate().getTime()) + "'");
			}
			...
			for (Object[] b : objectList) {
				...
				if (b[2] == null) {
					...
					if (claimRole.equals("DISABILITY")) {
						ProposalInsuredPerson proposalInsuredPerson = lifeProposalDAO.findProposalInsuredPersonById((String) b[2]);
						...
						PolicyInsuredPerson policyInsuredPerson = lifePolicyInsuredPersonDAO.findById((String) b[2]);
				...
				id = (String) b[4];
				resultMap.put(id, new LCL001(id, policyNo, claimRole, insuredPerson, sumInsured, null, submittedDate));
			}
			em.flush();
