...
public class LifeProductOverviewDAO extends BasicDAO implements ILifeProductOverviewDAO {
	...
	public LifeProductOverview findLifePolicyByTownship() {
		...
		try {
			...
			lifeQuery.append("Select i.residentAddress.township.name, Count(f.id) From LifePolicy f inner join f.policyInsuredPersonList i  Group By i.residentAddress.township.name");
	...
	public LifeProductOverview findLifePolicyByProductType() {
		...
		try {
			logger.debug("find() method has been started.");
				...
				query.append("Select pro.name, count(Distinct(p.id)) from LifePolicy p inner join p.policyInsuredPersonList pb , Product pro  where p.policyNo is not null and pb.product.id = pro.id group by pro.name");
	...
	public LifeProductOverview findLifePolicyByGender() {
		...
		try {
			logger.debug("find() method has been started.");
			StringBuffer lifeQuery = new StringBuffer();
			lifeQuery.append("Select pi.gender, count(l.id) from LifePolicy l inner join l.policyInsuredPersonList pi where l.id = pi.lifePolicy.id and l.policyNo is not null group by pi.gender") ;
			Query q = em.createQuery(lifeQuery.toString());
			List<Object []> raws = q.getResultList();
        	for (Object [] a: raws) {
	...
	public LifeProductOverview findLifePolicyByMonth(int year) {
		...
		try {
			logger.debug("find() method has been started.");
			StringBuffer lifeQuery = new StringBuffer();
			lifeQuery.append("Select count(f.id) from LifePolicy f where f.policyNo is not null AND f.commenmanceDate > :startDate " +
            		"AND f.commenmanceDate < :endDate");
			for(int i = 1 ; i < 13 ; i++){
				Query q = em.createQuery(lifeQuery.toString());
